  ...
  try {
    ...
    if (!allowedTypes.has(file.type) && !isAllowedExt) {
      return NextResponse.json(
        { error: "Only JPG, PNG, or WEBP images are accepted for profile pictures." },
        { status: 400 }
      );
    }

    // 2. File Size Validation (<= 2 MB)
    const maxSizeBytes = 2 * 1024 * 1024; // 2 MB
    if (file.size > maxSizeBytes) {
      const actualMb = (file.size / (1024 * 1024)).toFixed(1);
      return NextResponse.json(
        { error: `File size exceeds the 2 MB limit. Your file is ${actualMb} MB. Please upload a smaller file.` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Unable to process profile picture upload", details: error?.message || String(error) },
      { status: 500 }
    );
  }
}
